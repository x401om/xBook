...
- (id)init {
  self = [[EPubViewController alloc]initWithNibName:@"EPubView" bundle:[NSBundle mainBundle]];
...
- (void)viewDidLoad {
  ...
  UILongPressGestureRecognizer *longGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPress:)];
  [webView addGestureRecognizer:longGestureRecognizer];
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getTranslation) name:UIMenuControllerDidShowMenuNotification object:nil];
...
- (void)getTranslation {
  ...
  XBTranslatePopover *popover = [[XBTranslatePopover alloc]initWithSource:selection];
